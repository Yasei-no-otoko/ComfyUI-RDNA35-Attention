...
| backend | Sampler median | Prompt median |
| --- | ---: | ---: |
| Flash Attention | 76.08 s | 78.54 s |
| PISA 23/144 | 65.94 s | 68.38 s |

同一resident processでFlash、PISA、PISA、Flashの順に測定し、Samplerを13.3%、Prompt totalを12.9%短縮しました。runtime verifierでは1 forwardにつき対象self-attention 24/24、cross-attention 0、fallback 0です。旧0.7%表はPISA未実行のFlash fallbackを誤って比較していたため無効です。PISAは近似attentionなので、同seed画像はFlashと完全一致しません。測定画像は破綻せず、Flash比でSSIM 0.961379、RGB cosine 0.999484でした。近似差を許容する明示opt-in用途として扱います。
